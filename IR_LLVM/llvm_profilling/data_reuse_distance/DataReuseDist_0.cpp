...
struct DataReuseDist : public ModulePass {
    ...
    bool runOnModule(Module &M) override {
        ...
        if (mainFunc) {
            ...
            FunctionCallee prt = M.getOrInsertFunction("printInstrReuseDist",
    ...
    bool runOnFunction(Function &F) {
        ...
        for (Function::iterator B = F.begin(); B != F.end(); ++B) {
            for (BasicBlock::iterator I = B->getFirstInsertionPt(); I != B->end(); ++I) {
                ...
                Instruction& inst = *I;
                // Get the num of operands for the inst
                unsigned int opnt_cnt = inst.getNumOperands();
                // pass through all operands
                for (unsigned int  i = 0 ; i < opnt_cnt; ++i) {
                    // llvm::outs() << "\t\t\t";
                    Value *opnd = inst.getOperand(i);
                    // llvm::outs() << opnd->getType()->isLabelTy();
                    // llvm::outs() << opnd->getType()->isPointerTy();
                    // llvm::outs() << opnd << " " << opnd->getValueID() << " " << opnd->getValueName();
                    // llvm::outs() << "[" << opnd << " " << opnd->getNumUses() << 
                    //     " islabel:" << opnd->getType()->isLabelTy() << " " 
                    //     // " issinglevalue:" << opnd->getType()->isSingleValueType() << " "
                    //     // " Type:" << opnd->getValueID() << " " << llvm::Value::ValueTy::ConstantIntVal << " " <<
                    //     << isa<Constant>(opnd)
                    //     // << *opnd 
                    //     << "]      ";
                    // check if Constant opnd or label type. If not, callback
                    if (!isa<Constant>(opnd) && !opnd->getType()->isLabelTy()) {
                    // if (opnd->getType()->isPointerTy()) {
                        std::vector<Value *> args = {
                            ConstantInt::get(Type::getInt32Ty(F.getParent()->getContext()),
                                            reinterpret_cast<std::intptr_t>(dyn_cast<Value>(opnd)))
                        };
                        IRBuilder<> Builder(&*I);
                        Builder.CreateCall(ist, args);
                        unsigned long out = (unsigned long)opnd;
                        std::cout << "call operand " << std::dec << out << "  ";
                    }
                }
                // llvm::outs() << "[Inst id " << &inst << "] ";
                // llvm::outs() << inst.getOpcodeName() << "\t";
                // llvm::outs() << inst.isSafeToRemove();
                // llvm::outs() << inst.isAssociative();
                // llvm::outs() << (Value *)(&inst)->getNumUses() << " " << (Value *)(&inst)->hasNUsesOrMore(1) << " ||  ";
                if ((&inst)->hasNUsesOrMore(1)) {
                    std::vector<Value *> args = {
                        ConstantInt::get(Type::getInt32Ty(F.getParent()->getContext()),
                                        reinterpret_cast<std::intptr_t>(&inst))
                    };
                    IRBuilder<> Builder(&*I);
                    Builder.CreateCall(ist, args);
                    unsigned long out = (unsigned long)&inst;
                    std::cout << "call retval " << std::dec << out << " ";
                }
                llvm::outs() << inst << "\t\t\t\t\t";
                std::cout << std::endl;
            }
...
static RegisterPass<DataReuseDist> X(DEBUG_TYPE, "Collecting Dynamic Instruction Counts");
